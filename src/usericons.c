...
void read_globicons()
{
	...
	path = choices_find_xdg_path_load("globicons", NULL);
	if (!path)
...
static void write_globicons(void)
{
	...
	save = choices_find_xdg_path_save("globicons", NULL, TRUE);
				  ...
				  gboolean just_media)
{
	...
	target = choices_find_xdg_path_save(leaf, "MIME-icons", TRUE);
	g_free(leaf);
